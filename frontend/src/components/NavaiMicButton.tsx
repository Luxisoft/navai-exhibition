...
export default function NavaiMicButton({
  ...
  const [backendHasApiKey, setBackendHasApiKey] = useState(hasBackendApiKey);
  const [state, setState] = useState<VoiceState>("idle");
  const [ariaMessage, setAriaMessage] = useState("");
  const [statusMessage, setStatusMessage] = useState("");

  const hasInputApiKey = apiKey.trim().length > 0;
  const hasAnyApiKey = backendHasApiKey || hasInputApiKey;
  ...
  useEffect(() => {
    setBackendHasApiKey(hasBackendApiKey);
  }, [hasBackendApiKey]);
