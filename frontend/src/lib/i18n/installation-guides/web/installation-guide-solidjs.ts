...
const SETUP_CODE = `npm create vite@latest navai-web-demo -- --template solid-ts
cd navai-web-demo
npm install
...
const VOICE_NAVIGATOR_CODE = `import { RealtimeSession } from "@openai/agents/realtime";
import {
  NavaiVoiceOrbDock,
  buildNavaiAgent,
...
export function VoiceNavigator(props: VoiceNavigatorProps) {
  ...
  let orbHost: HTMLDivElement | undefined;
  ...
  const start = async () => {
    ...
    try {
      ...
      setError("NAVAI could not start.");
      session?.close();
      session = null;
  ...
  return (
    <section class="voice-card">
      <div class="voice-copy">
        <span class="voice-eyebrow">SolidJS + NAVAI</span>
      ...
      <p class="status-banner" data-navai-status>
        {props.statusMessage ?? "Voice demo ready for local actions."}
      </p>
    </section>
  );
}`;

const APP_CODE = `import { Show, createSignal, onCleanup, onMount } from "solid-js";
...
function App() {
  const [currentPath, setCurrentPath] = createSignal(HOME_PATH);
    ...
    <main class="app-shell">
      <section class="app-frame">
        ...
        <section class="demo-column">
          ...
          <Show when={currentPath() === BILLING_PATH} fallback={<HomePanel />}>
            <BillingPanel />
          </Show>
        </section>
      </section>
...
const WEB_SOLIDJS_GUIDE_COPY: Record<LanguageCode, LocalizedGuideCopy> = {
  en: {
    description:
      "SolidJS app on Vite that keeps NAVAI in one persistent root while the Orb runs through a small React host.",
    setup: {
      title: "Step 1 Create the web project",
      description:
        "Scaffold the app with Vite and the Solid TypeScript template, then prepare `src/ai` and `src/components` for NAVAI.",
      bullets: [
        "This guide uses plain SolidJS on Vite, not SolidStart, so the voice session stays in one client runtime.",
      ],
    },
    install: {
      title: "Step 2 Install dependencies",
      description:
        "Install NAVAI packages plus `react` and `react-dom`; the Orb itself is a React component mounted inside SolidJS.",
      bullets: [
    ...
    runtime: {
      ...
      description:
        "Mount one `VoiceNavigator.tsx`, keep the shell in `src/App.tsx`, and switch between Home and Billing with hash navigation so the Orb never unmounts.",
      bullets: [
        "The guide uses `window.location.hash` instead of another router dependency to keep the demo reproducible.",
        "React and ReactDOM are used only to host `NavaiVoiceOrbDock` inside `VoiceNavigator.tsx`.",
      ],
    },
    test: {
  ...
  fr: {
    description:
      "Application SolidJS sur Vite qui garde NAVAI dans une racine persistante pendant que l Orb tourne dans un petit host React.",
    setup: {
      title: "Etape 1 Creer le projet web",
      description:
        "Generez l application avec Vite et le template TypeScript de Solid, puis preparez `src/ai` et `src/components` pour NAVAI.",
      bullets: [
        "Ce guide utilise SolidJS simple sur Vite, pas SolidStart, afin que la session vocale reste dans un seul runtime client.",
      ],
    },
    install: {
      title: "Etape 2 Installer les dependances",
      description:
        "Installez les paquets NAVAI ainsi que `react` et `react-dom` ; l Orb lui-meme est un composant React monte dans SolidJS.",
      bullets: [
    ...
    runtime: {
      ...
      description:
        "Montez un seul `VoiceNavigator.tsx`, gardez le shell dans `src/App.tsx` et passez entre Home et Billing avec une navigation hash afin que l Orb ne soit jamais detruit.",
      bullets: [
        "Le guide utilise `window.location.hash` au lieu d une autre dependance de routeur pour garder une demo reproductible.",
        "React et ReactDOM servent uniquement a heberger `NavaiVoiceOrbDock` dans `VoiceNavigator.tsx`.",
      ],
    },
    test: {
  ...
  es: {
    description:
      "Aplicacion SolidJS sobre Vite que mantiene NAVAI en una raiz persistente mientras el Orb corre en un pequeno host de React.",
    setup: {
      title: "Paso 1 Crear el proyecto web",
      description:
        "Crea la app con Vite y el template TypeScript de Solid, luego prepara `src/ai` y `src/components` para NAVAI.",
      bullets: [
        "Esta guia usa SolidJS puro sobre Vite, no SolidStart, para que la sesion de voz viva en un solo runtime cliente.",
      ],
    },
    install: {
      title: "Paso 2 Instalar dependencias",
      description:
        "Instala los paquetes de NAVAI junto con `react` y `react-dom`; el Orb es un componente React montado dentro de SolidJS.",
      bullets: [
    ...
    runtime: {
      ...
      description:
        "Monta un solo `VoiceNavigator.tsx`, conserva el shell en `src/App.tsx` y cambia entre Home y Billing con navegacion hash para que el Orb nunca se desmonte.",
      bullets: [
        "La guia usa `window.location.hash` en lugar de otra dependencia de router para mantener la demo reproducible.",
        "React y ReactDOM se usan solo para hospedar `NavaiVoiceOrbDock` dentro de `VoiceNavigator.tsx`.",
      ],
    },
    test: {
  ...
  pt: {
    description:
      "Aplicativo SolidJS em Vite que mantem o NAVAI em uma raiz persistente enquanto o Orb roda em um pequeno host React.",
    setup: {
      title: "Etapa 1 Criar o projeto web",
      description:
        "Crie a aplicacao com Vite e o template TypeScript do Solid, depois prepare `src/ai` e `src/components` para o NAVAI.",
      bullets: [
        "Este guia usa SolidJS puro sobre Vite, nao SolidStart, para que a sessao de voz fique em um unico runtime cliente.",
      ],
    },
    install: {
      title: "Etapa 2 Instalar dependencias",
      description:
        "Instale os pacotes do NAVAI junto com `react` e `react-dom`; o Orb em si e um componente React montado dentro do SolidJS.",
      bullets: [
    ...
    runtime: {
      ...
      description:
        "Monte apenas um `VoiceNavigator.tsx`, mantenha o shell em `src/App.tsx` e alterne entre Home e Billing com navegacao por hash para que o Orb nunca seja desmontado.",
      bullets: [
        "O guia usa `window.location.hash` em vez de outra dependencia de roteador para manter a demo reproduzivel.",
        "React e ReactDOM sao usados apenas para hospedar `NavaiVoiceOrbDock` dentro de `VoiceNavigator.tsx`.",
      ],
    },
    test: {
  ...
  zh: {
    description:
      "基于 Vite 的 SolidJS 应用，将 NAVAI 保持在一个持久根节点中，Orb 通过一个很小的 React 宿主运行。",
    setup: {
      title: "第 1 步 创建 Web 项目",
      description:
        "使用 Vite 和 Solid TypeScript 模板创建应用，然后准备 NAVAI 需要的 `src/ai` 与 `src/components`。",
      bullets: [
        "本指南使用 Vite 上的纯 SolidJS，而不是 SolidStart，这样语音会话会一直留在同一个客户端运行时中。",
      ],
    },
    install: {
      title: "第 2 步 安装依赖",
      description:
        "安装 NAVAI 包以及 `react` 和 `react-dom`；因为 Orb 本身是挂载在 SolidJS 内部的 React 组件。",
    ...
    runtime: {
      ...
      description:
        "只挂载一个 `VoiceNavigator.tsx`，把壳层保留在 `src/App.tsx` 中，并用 hash 导航在 Home 与 Billing 间切换，这样 Orb 就不会被卸载。",
      bullets: [
        "本指南使用 `window.location.hash`，避免再引入一个路由依赖，并让示例更容易复现。",
        "React 和 ReactDOM 只用于在 `VoiceNavigator.tsx` 内承载 `NavaiVoiceOrbDock`。",
      ],
    },
    test: {
  ...
  ja: {
    description:
      "Vite 上の SolidJS アプリで、NAVAI を 1 つの永続ルートに保持し、Orb は小さな React ホスト経由で動作します。",
    setup: {
      title: "ステップ 1 Web プロジェクトを作成",
      description:
        "Vite の Solid TypeScript テンプレートでアプリを作成し、NAVAI 用に `src/ai` と `src/components` を準備します。",
      bullets: [
        "このガイドは SolidStart ではなく Vite 上の素の SolidJS を使うため、音声セッションは 1 つのクライアントランタイムに残ります。",
      ],
    },
    install: {
      title: "ステップ 2 依存関係をインストール",
      description:
        "NAVAI パッケージに加えて `react` と `react-dom` を入れます。Orb 自体は SolidJS 内でマウントする React コンポーネントです。",
      bullets: [
    ...
    runtime: {
      ...
      description:
        "`VoiceNavigator.tsx` を 1 回だけマウントし、シェルは `src/App.tsx` に残したまま、hash ナビゲーションで Home と Billing を切り替えても Orb がアンマウントされないようにします。",
      bullets: [
        "このガイドは `window.location.hash` を使い、追加のルーター依存を避けて再現しやすいデモにしています。",
        "React と ReactDOM は `VoiceNavigator.tsx` 内で `NavaiVoiceOrbDock` をホストするためだけに使います。",
      ],
    },
    test: {
  ...
  ru: {
    description:
      "Приложение SolidJS на Vite, где NAVAI живет в одном постоянном корне, а Orb работает через небольшой React-хост.",
    setup: {
      title: "Шаг 1 Создайте web-проект",
      description:
        "Создайте приложение через Vite с шаблоном Solid TypeScript, затем подготовьте `src/ai` и `src/components` для NAVAI.",
      bullets: [
        "Это руководство использует обычный SolidJS на Vite, а не SolidStart, чтобы голосовая сессия жила в одном клиентском runtime.",
      ],
    },
    install: {
      title: "Шаг 2 Установите зависимости",
      description:
        "Установите пакеты NAVAI, а также `react` и `react-dom`; сам Orb является React-компонентом, который монтируется внутри SolidJS.",
      bullets: [
    ...
    runtime: {
      ...
      description:
        "Смонтируйте один `VoiceNavigator.tsx`, оставьте shell в `src/App.tsx` и переключайтесь между Home и Billing через hash-навигацию, чтобы Orb никогда не размонтировался.",
      bullets: [
        ...
        "React и ReactDOM нужны только для размещения `NavaiVoiceOrbDock` внутри `VoiceNavigator.tsx`.",
      ],
    },
    test: {
  ...
  ko: {
    description:
      "Vite 기반 SolidJS 앱으로, NAVAI를 하나의 지속적인 루트에 유지하고 Orb는 작은 React 호스트를 통해 실행합니다.",
    setup: {
      title: "1단계 Web 프로젝트 생성",
      description:
        "Vite의 Solid TypeScript 템플릿으로 앱을 만들고, NAVAI용 `src/ai`와 `src/components`를 준비하세요.",
      bullets: [
        "이 가이드는 SolidStart가 아니라 Vite 위의 순수 SolidJS를 사용하므로 음성 세션이 하나의 클라이언트 런타임에 유지됩니다.",
      ],
    },
    install: {
      title: "2단계 의존성 설치",
      description:
        "NAVAI 패키지와 함께 `react`, `react-dom`을 설치하세요. Orb 자체는 SolidJS 안에 마운트되는 React 컴포넌트입니다.",
      bullets: [
    ...
    runtime: {
      ...
      description:
        "`VoiceNavigator.tsx`를 한 번만 마운트하고, shell은 `src/App.tsx`에 유지한 채 hash navigation으로 Home과 Billing 사이를 바꿔도 Orb가 언마운트되지 않게 하세요.",
      bullets: [
        "이 가이드는 `window.location.hash`를 사용해 추가 router 의존성을 피하면서 데모를 재현 가능하게 유지합니다.",
        "React와 ReactDOM은 `VoiceNavigator.tsx` 안에서 `NavaiVoiceOrbDock`를 호스팅할 때만 사용됩니다.",
      ],
    },
    test: {
  ...
  hi: {
    description:
      "Vite पर आधारित SolidJS ऐप जिसमें NAVAI एक स्थायी root में रहता है और Orb एक छोटे React host के जरिए चलता है।",
    setup: {
      title: "चरण 1 Web project बनाएं",
      description:
        "Vite के Solid TypeScript template से app बनाएं, फिर NAVAI के लिए `src/ai` और `src/components` तैयार करें।",
      bullets: [
        "यह guide SolidStart नहीं बल्कि Vite पर plain SolidJS उपयोग करती है, ताकि voice session एक ही client runtime में रहे।",
      ],
    },
    install: {
      title: "चरण 2 Dependencies install करें",
      description:
        "NAVAI packages के साथ `react` और `react-dom` install करें; Orb खुद SolidJS के अंदर mount होने वाला React component है।",
      bullets: [
    ...
    runtime: {
      ...
      description:
        "एक ही `VoiceNavigator.tsx` mount करें, shell को `src/App.tsx` में रखें, और Home तथा Billing के बीच hash navigation रखें ताकि Orb कभी unmount न हो।",
      bullets: [
        "Guide `window.location.hash` का उपयोग करती है ताकि अतिरिक्त router dependency से बचा जा सके और demo reproducible रहे।",
        "React और ReactDOM का उपयोग केवल `VoiceNavigator.tsx` के अंदर `NavaiVoiceOrbDock` host करने के लिए होता है।",
      ],
    },
    test: {
...
const WEB_SOLIDJS_GUIDE_TABS: Partial<
  Record<LanguageCode, InstallationGuideTab>
> = Object.fromEntries(
  LANGUAGES.map((language) => {
    const copy = WEB_SOLIDJS_GUIDE_COPY[language];
    ...
    return [
      language,
      {
        ...
        sections: buildSolidJsSections(language, copy),
      },
    ];
  }),
) as Partial<Record<LanguageCode, InstallationGuideTab>>;

export function getWebSolidjsGuideTab(
  language: LanguageCode,
): InstallationGuideTab {
  return getLocalizedInstallationGuideTab(WEB_SOLIDJS_GUIDE_TABS, language);
}
